...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-white leading-tight">Editar usuário</h2>
    </x-slot>
            ...
            <div class="panel rounded-xl p-6">
                <form method="POST" action="{{ route('admin.users.update', $user) }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input id="name" name="name" type="text" value="{{ old('name', $user->name) }}" required autofocus class="{{ $input }}">
                    ...
                    <div>
                        <label for="password" class="{{ $label }}">Nova senha (deixe em branco pra manter)</label>
                    ...
                    <label class="flex items-center">
                        <input type="checkbox" name="is_admin" value="1" class="rounded border-[#272c45] bg-[#161a2c] text-indigo-600 focus:ring-indigo-500" @checked(old('is_admin', $user->is_admin))>
                        <span class="ms-2 text-sm text-gray-300">Administrador (pode gerenciar usuários)</span>
                    </label>

                    <div class="flex items-center gap-3 pt-2">
                        <button class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-md">Salvar</button>
